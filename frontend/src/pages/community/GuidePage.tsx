...
const SECTIONS: GuideSection[] = [
  {
    ...
    icon: BookOpen,
    blocks: [
      { type: 'p', text: '这是一个"第二大脑"系统：把你看到的内容（网页、书摘、RSS、邮件）和自己的想法（笔记、便签）收进来，经过一条加工流水线变成可复用的知识卡片，再通过碰撞产生新想法，最后沉淀成你自己的知识体系。' },
      { type: 'p', text: '核心理念：知识不是存起来的，是生产出来的。所以站内功能按"采集 → 加工 → 沉淀 → 运用"组织，对应顶部四个一级菜单。' },
      { type: 'h', text: '双脑概念（先理解这个，后面都顺了）' },
      { type: 'list', items: [
        '个人脑：你自己的东西——笔记、便签、实操记录、复盘。',
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'list', items: [
        ...
        '第三步 · 抽：对卡片做"抽取"，AI 会提炼出核心概念和模型（这一步消耗 LLM 余额，几分钱）。',
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'list', items: [
        ...
        '抽取：AI 从卡片里提取核心概念/模型/行动建议（付费，几分钱/次）。',
        '碰撞：把两个概念放在一起让 AI 找联系、找矛盾、找杂交点（付费）。可以单个碰撞，也可以选中多个后批量碰撞。',
      ...
      { type: 'list', items: [
        '素材池：跨工具共享的灵感素材中转站。',
        '跨域联想 / 创意碰撞 / 概念杂交 / 反事实探索：四种 AI 创意发生器，玩法不同，都是输入概念输出新组合（付费）。',
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'list', items: [
        ...
        '验证中心：重要知识让多个 AI 模型交叉验证，标出可信度（付费）。',
        '来源追溯 / 可信度地图：按域名统计来源可靠性，分辨哪些网站的内容值得信。',
        '反证墙：集中审查有争议、可能被证伪的知识。',
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'list', items: [
        '每日复盘：每天花 5 分钟回顾今天的输入和产出，AI 帮你找行为差距（付费，建议每天用一次就够）。',
        '知识健康 / 进化轨迹 / 调用追踪：三条线看你的知识是在"收藏"还是在"内化"。',
        '实操记录：知识用了没有、效果如何，记在这里。"实践深度"就是这么积累的。',
        '真进化 vs 伪成熟：记录让你不舒服的事——进化发生在摩擦之后。',
        '认知势能：分析你的内容里哪些能下沉为习惯、能产出为作品、能变现（付费，内容积累多了再用）。',
        '关我屁事：粘贴一段外部内容，AI 判断它和你当前关注点相不相关，帮你做减法（付费）。',
        '认知镜像（思维指纹/认知偏差/脑侧冲突）：AI 分析你的思维模式（付费，点按钮才跑，不会自动扣费）。',
  ...
  },
  {
    ...
    icon: Coins,
    blocks: [
      { type: 'p', text: '站内标注"AI"的分析类功能会调用大模型，从余额扣费（通常是几分钱一次）：思维指纹、偏见检测、脑侧冲突、认知势能、每日复盘、关我屁事、验证中心、管线抽取/碰撞、深度检查的"AI 深度评估"模式等。' },
      { type: 'list', items: [
        '所有付费功能都是"点按钮才跑"，打开页面不会自动扣费（思维指纹、认知势能等都是手动触发）。',
        '内容深度检查默认是免费的"规则评估"，一分钱不花；想要更准的判断再切到"AI 深度评估"。',
        '右上角 LLM 控制台可以选择默认模型，便宜模型和贵的模型差价很大，日常用便宜的足够。',
        '余额、冻结金额、每笔消耗流水在「我的 → 充值/账单」页面都能查到，每一笔都有记录。',
      ]},
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      { type: 'list', items: [
        ...
        '余额怎么少了：去账单页看流水，每笔消耗都有任务类型和金额。分析类功能按次计费，页面不自动跑。',
        ...
        <div className="flex-1 min-w-0 space-y-6">
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1">
              从零上手第二大脑：核心概念、推荐流程、每个功能干什么、哪些要花钱。
            </p>
          </div>
