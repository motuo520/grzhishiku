...
const PRACTICE_TYPES = [
  { value: 'applied', label: '应用', color: 'text-blue-400 bg-blue-400/10' },
  { value: 'taught', label: '教授', color: 'text-purple-400 bg-purple-400/10' },
  { value: 'iterated', label: '迭代', color: 'text-emerald-400 bg-emerald-400/10' },
...
const PracticeRecordsPage: FC = () => {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="p-4 rounded-xl border border-white/[0.06] bg-bg-secondary mb-6 space-y-3">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-xs border transition-all ${filterType === 'all' ? 'bg-info/15 text-info border-info/30' : 'bg-white/[0.03] text-text-secondary border-white/[0.06]'}`}
          >
            全部
          </button>
          {PRACTICE_TYPES.map((t) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs border transition-all ${filterType === t.value ? `${t.color} border-current` : 'bg-white/[0.03] text-text-secondary border-white/[0.06]'}`}
            >
              {t.label}
            </button>
          ))}
      ...
      <div className="space-y-3">
        {filteredRecords?.map((record) => {
          ...
          return (
            <div key={record.id} className="p-4 rounded-xl border border-white/[0.06] bg-bg-secondary group">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-text-muted hover:text-danger hover:bg-danger/10 transition-colors disabled:opacity-50 shrink-0"
                  title="删除记录"
                >
