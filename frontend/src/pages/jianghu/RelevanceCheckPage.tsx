    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <textarea
          ...
          className="w-full px-4 py-3 rounded-xl bg-bg-secondary border border-white/[0.06] text-sm text-text-primary min-h-[160px]"
        />
        <textarea
          value={context}
          onChange={(e) => setContext(e.target.value)}
          placeholder="可选：补充你当前的上下文（最近关注什么、在解决什么问题）..."
          className="w-full px-4 py-3 rounded-xl bg-bg-secondary border border-white/[0.06] text-sm text-text-primary min-h-[80px]"
        />
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="px-3 py-2 rounded-lg bg-bg-secondary border border-white/[0.06] text-sm text-text-primary"
          >
      ...
      {check.data && (
        <div className="mt-6 rounded-xl border border-white/[0.06] bg-bg-secondary overflow-hidden">
          <div className="p-4 border-b border-white/[0.06]">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-10 h-10 rounded-lg bg-white/[0.03] flex items-center justify-center ${action?.color || 'text-info'}`}>
              ...
              <div className="pt-1">
                {action.path ? (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-lg bg-info/10 text-info hover:bg-info/20 transition-colors text-sm"
                  >
                    {action.label}
                    <ArrowRight className="w-4 h-4" />
                  </button>
                ) : (
                  <span className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white/[0.03] text-text-muted text-sm cursor-not-allowed">
