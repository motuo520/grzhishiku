...
const ExtractPage: FC = () => {
  ...
  const { items, isLoading: isItemsLoading, error: queryError, refetch } = usePipelineItems('extracted', brainSide);
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {[
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-xs border transition-all ${subtypeFilter === opt.key ? 'bg-info/15 text-info border-info/30' : 'bg-white/[0.03] text-text-secondary border-white/[0.08] hover:bg-white/[0.06]'}`}
              >
                {opt.label}
              </button>
            ))}
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 pt-3 border-t border-white/[0.06]">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-success to-emerald-500 text-white rounded-xl text-sm font-medium hover:shadow-[0_0_20px_rgba(35,134,54,0.4)] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
                  ...
                  {isConcept && (
                    <>
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white/[0.05] border border-white/[0.08] rounded-lg text-xs text-text-primary hover:bg-success/10 hover:border-success/30 hover:text-success transition-all disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-rose-400/10 border border-rose-400/30 rounded-lg text-xs text-rose-400 hover:bg-rose-400/20 transition-colors"
                        title="判断这个概念与你是否相关"
                      >
