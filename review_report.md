...
> | 序号 | 等级 | 状态 | 验证方式 |
> |------|------|------|----------|
> | 1 | 致命 | ✅ 已修复 | `admin_login` 现返回 `admin` 对象 + `access_token`（auth.py:93-104） |
> | 2 | 致命 | ✅ 已修复 | `User` 模型已添加 `username`/`display_name`（base.py:62-63） |
> | 3 | 致命 | ✅ 已修复 | 内容审核改查 `User` 表（content.py:36） |
> | 4 | 致命 | ✅ 已修复 | `AdminAuditLog` 已添加 `details` 字段（base.py:603） |
> | 5 | 严重 | ✅ 已修复 | 2026-07-16 已补空值检查（auth.py:84、gdpr.py:45,47） |
> | 6 | 严重 | ✅ 已修复 | 迁移文件与模型统一为 `extra_data` |
> | 7 | 严重 | ✅ 已修复 | `Payment.provider_order_id` 已添加（billing.py:96） |
> | 8 | 严重 | ✅ 已修复 | Webhook 异常改抛 500（billing.py:506-508） |
> | 9 | 严重 | ✅ 已修复 | Prometheus 端点已加 `require_permission` 认证 |
> | 10 | 警告 | ✅ 已解决 | 路由已重构嵌套至 ModuleLayout，无重复定义 |
> | 11 | 警告 | ✅ 已修复 | graph.py 已实现（459 行） |
> | 12 | 警告 | ✅ 已修复 | support.py 已实现（291 行） |
> | 13–23 | 警告/提示 | 🔲 未复核 | 低优先级，留待下次审查 |
>
> **遗留行动项**：#13–23 待复核（低优先级）。#1–#12 已全部修复并验证。
