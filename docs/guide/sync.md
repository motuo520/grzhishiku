...
个人第二大脑的「云同步」是付费层核心功能：订阅「存储会员」后，你的笔记、剪藏、知识单元和胶囊可以在多台设备间同步，且**服务器只存储密文**。
...
1. 订阅「存储会员」（9.9 元/月或 99 元/年）。
2. 进入「设置 → 同步」。
3. 设置一个强壮的同步密码，建议与登录密码不同。
4. 点击「立即上传快照」注册当前设备并上传加密数据。
5. 在另一台设备用同一账号登录，输入相同同步密码，点击「从云端恢复」。
