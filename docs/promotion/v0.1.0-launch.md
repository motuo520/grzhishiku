...
4. **多端同步**：数据在设备间同步，端到端加密，服务器只看密文（开源版免费开放）。
...
无需任何 `.env` 配置即可启动；如需覆盖默认值，可在仓库根目录自建 `.env`（compose 会自动读取其中的变量）。
...
- Encrypted cloud sync: free multi-device sync; the server only sees ciphertext.
...
No `.env` setup is required; to override defaults, create a root `.env` (compose reads it automatically).
